...
namespace Sds.Blob.Storage.WebApi
{
    ...
    public class BlobsController : Controller
    {
        ...
        private Guid? UserID
        {
            get
            {
                if (User.Claims.Where(c => c.Type == "clientId").Any())
                {
                    return null;
                }

                return User.Claims.Where(c => c.Type.Contains("/nameidentifier")).Any() ? (Guid?)Guid.Parse(User.Claims.Where(c => c.Type.Contains("/nameidentifier")).Single().Value) : null;
            }
        }
